...
class Controller():
    def __init__(self):
        ...
        self.fingerData = {} #Voor het opslaan van de coördinaten van de vingers
        self.preventDoubleTouch = False #Voorkomen dat de speler een knop ingedrukt kan houden
    ...
    def resetGame(self):
        ...
        player.rect.y = -100
    def checkInput(self):
        pygame.event.set_allowed([pygame.QUIT, pygame.FINGERDOWN, pygame.FINGERUP])
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.isGameFinished = True
            elif event.type == pygame.FINGERDOWN:
                fingerX = event.x * WIDTH
                fingerY = event.y * HEIGHT
                self.fingerData[event.finger_id] = fingerX, fingerY
            elif event.type == pygame.FINGERUP:
                self.fingerData.pop(event.finger_id)
                self.preventDoubleTouch = False
    def update(self):
        self.checkInput()
...
class GM_MainMenu():
    def checkInput(self):
        for finger, pos in controller.fingerData.items():
            if (buttonPlay.isTouched):
                controller.preventDoubleTouch = True
...
class GM_ChallengesMenu():
    ...
    def checkInput(self):
        for finger, pos in controller.fingerData.items():
            if (buttonBackToMenu.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(MAIN_MENU)
            else:
                for n in range (len(buttonChallenge)):
                    if (buttonChallenge[n].isTouched and controller.challengeLock[n] == False):
                        controller.preventDoubleTouch = True
    ...
    def checkInput(self):
        for finger, pos in controller.fingerData.items():
            if (buttonBackToMenu.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(MAIN_MENU)
            elif (buttonLevelEditor.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(LEVEL_EDITOR)
            elif (buttonBack.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(CHALLENGES_MENU)
            elif (buttonInstructions.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(INSTRUCTIONS)
            elif (buttonNext.isTouched):
    ...
    def checkInput(self):
        for finger, pos in controller.fingerData.items():
            for n in range(len(self.movements)):
                if (buttonNextMovement[n].isTouched):
                    controller.preventDoubleTouch = True
                    if (self.movementsIndex[n] == 3):
                        self.movementsIndex[n] = 0
                    else:
                        self.movementsIndex[n] += 1
                elif (buttonPreviousMovement[n].isTouched):
                    controller.preventDoubleTouch = True
                    if (self.movementsIndex[n] == 0):
                        self.movementsIndex[n] = 3
                    else:
                        self.movementsIndex[n] -= 1
            if (buttonNext.isTouched and controller.challengeLock[controller.challengeIndex + 1] == False):
                controller.preventDoubleTouch = True
                controller.setChallenge(controller.challengeIndex + 1)
                controller.setGameMode(INSTRUCTIONS)
            elif (buttonBack.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(CHALLENGES_MENU)
            elif (buttonBackToMenu.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(MAIN_MENU)
    def draw(self):
    ...
    def checkInput(self):
        for finger, pos in controller.fingerData.items():
            if (buttonBackToMenu.isTouched):
                controller.preventDoubleTouch = True
                controller.setGameMode(MAIN_MENU)
            elif (buttonSaveAndExit_GE.isTouched):
            ...
            elif (buttonIncreaseGridX.isTouched):
                controller.preventDoubleTouch = True
                controller.resizeGridBorder(INCREASE, X)
            elif (buttonDecreaseGridX.isTouched):
                controller.preventDoubleTouch = True
                controller.resizeGridBorder(DECREASE, X)
            elif (buttonIncreaseGridY.isTouched):
                controller.preventDoubleTouch = True
                controller.resizeGridBorder(INCREASE, Y)
            elif (buttonDecreaseGridY.isTouched):
                controller.preventDoubleTouch = True
...
class GM_LevelEditor():
    ...
    def reset(self):
        ...
        controller.clearMap()
    def checkInput(self):
        if (not controller.fingerData):
            self.changeItemBalance = True
        else:
            for finger, pos in controller.fingerData.items():
                gridX = (math.floor(pos[X] / BLOCK_WIDTH))
                if (gridX > (GRID_WIDTH - 1)):
                    gridX = GRID_WIDTH - 1
                gridY = (math.floor(pos[Y] / BLOCK_HEIGHT))
                ...
                if (buttonBackToMenu.isTouched):
                    controller.preventDoubleTouch = True
                    controller.setGameMode(MAIN_MENU)
                elif (buttonSaveAndExit_LE.isTouched):
                    controller.preventDoubleTouch = True
                    controller.setGameMode(BIBBER)
                elif (buttonItemPrevious.isTouched):
                    controller.preventDoubleTouch = True
                    cursor.setItem(PREVIOUS)
                elif (buttonItemNext.isTouched):
                    controller.preventDoubleTouch = True
                    cursor.setItem(NEXT)
                elif (pos[Y] < (HEIGHT - (2 * BLOCK_HEIGHT)) and gridX >= (controller.gridBorderLeft[X]) and gridX <= (GRID_WIDTH - controller.gridBorderRight[X] - 1)
...
class Button(pygame.sprite.Sprite):
    ...
    def checkTouch(self):
        for finger, pos in controller.fingerData.items():
            if (pos[X] > self.rect.x and pos[X] < (self.rect.x + self.width) and
                pos[Y] > self.rect.y and pos[Y] < (self.rect.y + self.height) and
                not controller.preventDoubleTouch):
                controller.preventDoubleTouch = True
                self.isTouched = True
            else:
                self.isTouched = False
...
class Player(pygame.sprite.Sprite):
    ...
    def updateTopDown(self):
        if (controller.challengeIndex > 1):
            ...
            self.dy = 0
            for finger, pos in controller.fingerData.items():
                if (pos[X] > 300 and pos[X] < 600 and 
                    pos[Y] < ((GRID_HEIGHT * BLOCK_HEIGHT) / 2)):
                    self.direction = UP
                    self.dy = -SPEED
                elif (pos[X] > 300 and pos[X] < 600 and 
                    pos[Y] > ((GRID_HEIGHT * BLOCK_HEIGHT) / 2) and pos[Y] < (GRID_HEIGHT * BLOCK_HEIGHT)):
                    self.direction = DOWN
                    self.dy = SPEED
                elif (pos[X] < ((GRID_WIDTH * BLOCK_WIDTH) / 3) and
                      pos[Y] < (GRID_HEIGHT * BLOCK_HEIGHT)):
                    self.direction = LEFT
                    self.dx = -SPEED
                elif (pos[X] > ((GRID_WIDTH * BLOCK_WIDTH) / 3) and
                      pos[Y] < (GRID_HEIGHT * BLOCK_HEIGHT)):
